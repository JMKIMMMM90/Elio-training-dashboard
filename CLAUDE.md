...
- 데이터는 Firebase Realtime Database에 사람별로 실시간 저장.
  ...
  - 제출 가점(submitBonus, 사다리): 마감(주차 마지막 날 10:00) 기준 2일 전 +5 /
    전날 +3 / 정시 0 / 지각 -5. 마감 후 미제출도 -5(역전 방지). PPT·서평 평균(weekBonus).
  - 신입 화면 실시간 점수 원칙: 자동 30점+가점은 평가 전에도 "실시간 N점"으로 항상 표시
    (내 점수 카드 + 주차 추이 바 + 오늘의 미션 스트립의 제출 카운트다운).
...
- 화면 구조(하단 고정 알약 네비게이션, view 상태): 신입 = 홈(미션 스트립·Road 게이지·
  오늘의 할 일·이번 주 제출함·최근 코멘트) / 교육안내(목적·GROUND RULES·평가 기준·커리큘럼) /
  목표관리(달력↔주차별 세그먼트, planMode) / 내 평가(점수 상세·추이).
  관리자·담당자 = 현황 / 평가 / 관리(관리자만) / 안내 — 접속 시 현황에서 시작.
- 네비 순서(신입): 홈 → 목표관리 → 교육안내 → 내 평가. 홈에 내 점수 요약 카드
  (실시간, 오늘의 할 일 위), 내 평가 탭 = 항목별 상세 + 코멘트.
- 교육안내 구성(스토리 순서): 교육 목적(역량 4개: 인내·성실·목표관리·기본기, 담백한 톤)
  → 3개 과정 카드(과정 목표·배점 연결, PPT 목표 워딩에 AI 언급 없음)
  → 평가 구조 시각화(500 스택 바 + 주차 100 구성 바 + 가점: 자정 기준 명시)
  → GROUND RULES → 커리큘럼.
...
## 폴더 구조·기록 규칙
- index.html(앱) / CLAUDE.md(Claude용 규칙·스펙, 수정 시 여기부터 갱신) /
  README.md(사람용 소개) / docs/개발일지.md(굵직한 변경만, 최신이 위) /
  docs/회고.md(작업 방식·배운 점, 성과발표용) / .claude/skills/(프로젝트 스킬).
- 굵직한 기능이 배포되면 docs/개발일지.md에 한 줄 추가한다. 세부는 git 커밋이 담당.
- ★이 저장소는 public(GitHub Pages). 회사 원본 자료는 절대 커밋하지 않는다.
  원본은 저장소 폴더 안 `교육자료/`에 있으며 .gitignore로 git에서 제외되어 있다
  (git add -f 등으로 강제 추가 금지).
  - 쉐도잉 원본 PPT: 교육자료/전달용/<N>주차/
  - 원본 보고서·퀴즈(문제/정답): 교육자료/☆주차별 교육과제_20260901/주차별 교육과제_202402/
  - 표준 서식: 교육자료/2025 보고서·발표자료 마스터파일 표준 서식.pptx
  - 퀴즈 "정답/답안" 파일은 특히 민감 — 신입에게 전달되는 산출물에 포함 금지.
- 저장소 로컬 경로: C:\Users\김재민\Documents\Work\R&D\Elio-training-dashboard_2609
