...
- 데이터는 Firebase Realtime Database에 사람별로 실시간 저장.
  ...
  - 평가(주차 100점 + 조기제출 가점 최대 5): PPT 40(달성률 20 자동 + 정확성 20) /
    독후감 30(독창성 10 + 달성률 10 자동 + 논리 10) / 보고서 30(TEST 30).
    자동 항목은 calcWeekReview·weekAreaRate로 렌더링 시 계산(저장 안 함).
    담당자 입력은 users/<신입>/reviews/w{n} = {acc, orig, logic, test, comment, by, at},
    reviews/final = {comment}(종합점수는 평가된 주차 평균 자동). 저장 즉시 신입 본인에게 공개.
  - 조기제출 가점(submitBonus, 사다리): 마감(주차 마지막 날 10:00) 기준 2일 전 +5 /
    전날 +3 / 정시 +1 / 지각 0. PPT·독후감 두 파일 평균(weekBonus). 파일 배지로도 표시.
